...
/**
 * CollectiveMapper for finding centroids
 */

public class MiniBatchKmeansMapper extends CollectiveMapper<String, String, Object, Object> {
    ...
    protected void setup(Context context)
        ...
        batchSize = configuration.getInt(MBKmeansConstants.BATCH_SIZE, 10);
    ...
    protected void mapCollective( KeyValReader reader, Context context) throws IOException, InterruptedException {
        ...
        runMiniBatchKmeans(pointFiles, conf, context);
    ...
    private void runMiniBatchKmeans(List<String> fileNames, Configuration conf, Context context) throws IOException {
        // -----------------------------------------------------
        // Load centroids
        ...
        if (this.isMaster()) {
            loadCentroids(cenTable, vectorSize, conf.get(MBKmeansConstants.CFILE), conf);
        }
        ...
        // ArrayList<DoubleArray> batchPointList = null;
        ...
        for(int iter=0; iter < iteration; iter++){
            ...
            // sample batches randomly from data points list
            ArrayList<DoubleArray> batchPointList = new ArrayList<>();
            for(int i = 0; i < batchSize; i++) {
                val = r.nextInt(dataPoints.size());
                batchPointList.add(dataPoints.get(val));
            }
            ...
            computation(cenTable, previousCenTable, batchPointList);
    ...
    private void calculateCentroids(Table<DoubleArray> cenTable, Table<DoubleArray> previousCenTable){
        for( Partition<DoubleArray> part_i: cenTable.getPartitions()){

            Partition<DoubleArray> part_i_1 = previousCenTable.getPartition(part_i.id());

            double[] doubles_i = part_i.get().get();
            double[] doubles_i_1 = part_i_1.get().get();

            doubles_i[vectorSize] += doubles_i_1[vectorSize];

            for (int h = 0; h < vectorSize; h++) {
                doubles_i[h] = (doubles_i_1[h]*doubles_i_1[vectorSize] + doubles_i[h]) / doubles_i[vectorSize];
            }
            System.out.println("after calculate new centroids");
            printTable(cenTable);
        }
    ...
    //This method samples batches randomly from data points list
    /*private ArrayList<DoubleArray> batchGeneration(ArrayList<DoubleArray> dataPoints) {
        ArrayList<DoubleArray> batchList = new ArrayList<>();
        java.util.Random r = new java.util.Random();

        for(int i = 0; i < batchSize; i++) {
            int val = r.nextInt(dataPoints.size());
            batchList.add(dataPoints.get(val));
        }
